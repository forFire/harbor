...
//		logger.info(" Leader convert message begin ：[" + hexs + "]");
		// @@##区分 没有的话就不分
...
//		logger.info("  convert message end ：[" + message + "]");
		return message;
	}
...
//			String binaryString1 = toString(Integer.toBinaryString(s1), 8);
		...
		BaseMessage ret = new BaseMessage(Act.LEADER, systemStatus);
		return ret;

	}

	// 2建筑消防设施部件运行状态
	public BaseMessage deviceStatus(StringUtil str, String primaryAddress,String lastTime) throws Exception {
		logger.info("建筑消防设施部件运行状态（火警或故障信息）============================");
		FireAlarm fireAlarm = new FireAlarm();
		
		fireAlarm.setAlarmTime(DateUtil.dateStr(new Date(), "yyyyMMddHHmmss"));
		...
		logger.info("部件地址(探测器编码)设备编码=" +toString((primaryAddress), 12)+ (toString(str2, 5) + toString(str1,3)));
		// 部件地址=源地址+部件地址
		fireAlarm.setSn(toString((primaryAddress), 12) +  toString(str2, 5) + toString(str1,3));
		// 部件状态 2字节 低字节先传 2
		String componetStatus = "";
		// 低
		String s0 = str.getStr(2);
		// 高
		String s1 = str.getStr(2);

		componetStatus = toString(s1,2)+ toString(s0,2);
		
		// logger.info("部件类型="+componetStatus);
		// 火警保存到数据库是1 需要关联到报警类型表
		if ( componetStatus.equals("8002")) {
			fireAlarm.setType("in002");
		}
		// 故障保存到数据库是0
		else if ( componetStatus.equals("8004")) {
		...
		logger.info("部件状态编码=" +  componetStatus);
...
//		logger.info("componetMsg======" + componetMsg);
		...
		BaseMessage ret = new BaseMessage(Act.LEADER, fireAlarm);
//		logger.info("----------------------------------"+time);
		return ret;
	}
	...
	public BaseMessage devicOperation(StringUtil str, String primaryAddress) throws Exception {
		...
		deviceOperation.setPrimaryAddress(toString((primaryAddress), 12));
		...
		// 系统地址
		String sysAddress = str.getStr(2);
		deviceOperation.setSysAddress(toString(sysAddress,2));
		logger.info("系统地址========>" + toString(sysAddress,2));

		//本来官方定义多个状态位用的，利达修改成事件位
		String operationFlag = str.getStr(2);
		
		if("81".equalsIgnoreCase(operationFlag)){
			operationFlag = "0";
		}
		
		if("82".equalsIgnoreCase(operationFlag)){
			operationFlag = "1";
		}
		...
		BaseMessage ret = new BaseMessage(Act.LEADER, deviceOperation);
...
//		logger.info("装置运行状态===" + mainElectricFault);
		...
		BaseMessage ret = new BaseMessage(Act.LEADER, "");
...
//		String selfChecking = toString(Integer.toBinaryString(s11), 8);
		...
		BaseMessage ret = new BaseMessage(Act.LEADER, userDeviceOperation);
		return ret;
